	...
	private void initialize() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (yourAccount != null) {
						...
						Caller caller = new Caller(yourAccount.getName(), yourAccount.getUserIP());
		...
		btnVideoCall.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (yourAccount != null) {
					VideoCall videoCall = new VideoCall(yourAccount.getName(), yourAccount.getUserIP());
						...
						if (message.equals(Config.tokenVoiceCall)) {
							for (int i = 0; i < listModelUser.size(); i++) {
								...
								if (user.getName().equals(senderName)
									...
									Caller caller = new Caller(user.getName(), user.getUserIP());
									caller.setVisibleFrameCall(true);
							...
							for (int i = 0; i < listModelUser.size(); i++) {
								...
								if (user.getName().equals(senderName)
										&& senderName.equals(myAccount.getName()) == false) {
									VideoCall videoCall = new VideoCall(user.getName(), user.getUserIP());
